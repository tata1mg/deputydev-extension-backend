...
class TestAnthropicConversationTurnConversion:
    ...
    async def test_get_anthropic_conversation_turn_from_user_conversation_turn_with_cache_breakpoint(
        self,
        anthropic_provider: Anthropic,
        user_conversation_turn_with_cache_breakpoint: UserConversationTurn
    ) -> None:
    ...
    async def test_get_anthropic_conversation_turns_from_conversation_turns_mixed(
        self,
        anthropic_provider: Anthropic,
        mixed_unified_conversation_turns: List[UnifiedConversationTurn]
    ) -> None:
    ...
    async def test_get_anthropic_conversation_turns_from_conversation_turns_complex(
        self,
        anthropic_provider: Anthropic,
        complex_unified_conversation_turns: List[UnifiedConversationTurn]
    ) -> None:
...
class TestAnthropicRegionAndServiceClient:
    ...
    async def test_get_service_client_and_model_name_client_caching(
        ...
        with patch('app.backend_common.services.llm.providers.anthropic.llm_provider.BedrockServiceClient') as mock_bedrock:
...
class TestAnthropicNonStreamingResponse:
    ...
    async def test_parse_non_streaming_response_text_only(
        ...
        result = await anthropic_provider._parse_non_streaming_response(
            sample_invoke_model_response
        )
    ...
    async def test_parse_non_streaming_response_with_tool_use(
        self,
        anthropic_provider: Anthropic,
        tool_use_invoke_model_response: Dict[str, Any]
    ) -> None:
        """Test parsing non-streaming response with tool use."""
        result = await anthropic_provider._parse_non_streaming_response(
            tool_use_invoke_model_response
        )
    ...
    async def test_call_service_client_non_streaming(
        ...
        with patch.object(anthropic_provider, '_get_model_config', return_value=sample_model_config):
            with patch.object(anthropic_provider, '_get_service_client_and_model_name') as mock_get_client:
                ...
                mock_get_client.assert_called_once_with(session_id, LLModels.CLAUDE_3_POINT_5_SONNET, sample_model_config)
...
class TestAnthropicTokenCounting:
    ...
    async def test_get_tokens(
        self,
        anthropic_provider: Anthropic,
        mock_tiktoken_client: MagicMock
    ) -> None:
        """Test get_tokens method."""
        with patch('app.backend_common.services.llm.providers.anthropic.llm_provider.TikToken', return_value=mock_tiktoken_client):
    ...
    def test_extract_payload_content_for_token_counting_complex_system(
        self,
        anthropic_provider: Anthropic,
        complex_llm_payload_for_token_counting: Dict[str, Any]
    ) -> None:
        """Test payload content extraction with complex system message."""
        result = anthropic_provider._extract_payload_content_for_token_counting(
            complex_llm_payload_for_token_counting
        )
    ...
    def test_extract_payload_content_for_token_counting_malformed(
        self,
        anthropic_provider: Anthropic,
        malformed_llm_payload_for_token_counting: Dict[str, Any]
    ) -> None:
    ...
    def test_extract_payload_content_for_token_counting_empty_payload(
        self,
        anthropic_provider: Anthropic
    ) -> None:
    ...
    def test_extract_payload_content_for_token_counting_no_messages(
        self,
        anthropic_provider: Anthropic
    ) -> None:
...
class TestAnthropicEdgeCases:
    ...
    @pytest.mark.asyncio
    async def test_conversation_turn_conversion_empty_content(
        self,
        anthropic_provider: Anthropic
    ) -> None:
    ...
    @pytest.mark.asyncio
    async def test_region_selection_with_single_region(
        self,
        anthropic_provider: Anthropic
    ) -> None:
    ...
    @pytest.mark.asyncio
    async def test_parse_non_streaming_response_empty_content(
        self,
        anthropic_provider: Anthropic
    ) -> None:
    ...
    def test_extract_payload_content_handles_none_values(
        self,
        anthropic_provider: Anthropic
    ) -> None:
