...
class TestOpenAIUnifiedConversationTurns:
    ...
    async def test_get_openai_response_input_params_single_user_turn(
        ...
        result = await openai_provider._get_openai_response_input_params_from_conversation_turns(single_user_turn)
    ...
    async def test_get_openai_response_input_params_complex_conversation(
        ...
        tool_outputs = [
            item for item in result if isinstance(item, dict) and item.get("type") == "function_call_output"
        ]
...
class TestOpenAIServiceClientCalls:
    ...
    async def test_call_service_client_non_streaming_text(
        ...
        with patch("deputydev_core.llm_handler.providers.openai.llm_provider.OpenAIServiceClient") as mock_client:
            ...
            result = await openai_provider.call_service_client(
                session_id=123, llm_payload=mock_llm_payload, model=LLModels.GPT_4O, stream=False, response_type="text"
            )
    ...
    async def test_call_service_client_streaming(
        ...
        with patch("deputydev_core.llm_handler.providers.openai.llm_provider.OpenAIServiceClient") as mock_client:
            ...
            result = await openai_provider.call_service_client(
                session_id=123, llm_payload=mock_llm_payload, model=LLModels.GPT_4O, stream=True
            )
    ...
    async def test_call_service_client_default_response_type(
        ...
        with patch("deputydev_core.llm_handler.providers.openai.llm_provider.OpenAIServiceClient") as mock_client:
            ...
            result = await openai_provider.call_service_client(
                session_id=123, llm_payload=mock_llm_payload, model=LLModels.GPT_4O
            )
...
class TestOpenAITokenCounting:
    ...
    async def test_get_tokens_simple_content(
        ...
        with patch("deputydev_core.llm_handler.providers.openai.llm_provider.TikToken") as mock_tiktoken:
            ...
            result = await openai_provider.get_tokens(content=sample_content_for_token_counting, model=LLModels.GPT_4O)
    ...
    async def test_get_tokens_empty_content(
        ...
        with patch("deputydev_core.llm_handler.providers.openai.llm_provider.TikToken") as mock_tiktoken:
            ...
            result = await openai_provider.get_tokens(content="", model=LLModels.GPT_4O)
...
class TestOpenAIStreamingResponseParsing:
    ...
    async def test_parse_streaming_response_basic(
        ...
        result = await openai_provider._parse_streaming_response(
            mock_streaming_response, stream_id="test_stream_123", session_id=456
        )
...
class TestOpenAIIntegrationEdgeCases:
    ...
    async def test_large_payload_content_extraction_performance(
        ...
        large_payload = {
            ...
            "conversation_messages": [
                {"role": "user", "content": [{"type": "input_text", "text": f"Message {i} " * 50}]} for i in range(100)
            ],
            "tools": [
                {
                    "name": f"tool_{i}",
