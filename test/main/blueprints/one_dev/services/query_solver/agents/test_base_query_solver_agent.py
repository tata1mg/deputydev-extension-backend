...
class TestQuerySolverAgent:
    ...
    def test_generate_conversation_tool_from_client_tool_unsupported(
        self, unsupported_client_tool: ClientTool
    ) -> None:
    ...
    @pytest.mark.asyncio
    async def test_convert_text_agent_chat_to_conversation_turn_user(
        self, agent_chat_user_text: AgentChatDTO
    ) -> None:
    ...
    async def test_get_all_chat_attachments(
    ...
    async def test_get_conversation_turns_and_previous_queries_with_new_query(
        ...
        with patch.object(agent, 'get_all_chat_attachments', return_value=[]) as mock_get_attachments:
            ...
            mock_handler_instance.get_relevant_previous_agent_chats_for_new_query.assert_called_once_with(agent_chat_user_text)
    ...
    async def test_get_conversation_turns_and_previous_queries_missing_new_query_chat(
        ...
        with pytest.raises(ValueError, match="new_query_chat must be provided when payload.query is present"):
            await agent._get_conversation_turns_and_previous_queries(
                base_query_solver_input, mock_client_data, None
            )
    ...
    async def test_get_llm_inputs_and_previous_queries(
        ...
        from app.backend_common.services.llm.dataclasses.main import UserAndSystemMessages, NonStreamingResponse
