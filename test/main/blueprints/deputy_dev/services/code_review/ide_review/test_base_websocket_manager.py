...
class TestBaseWebSocketManagerInitializeAwsClient:
    ...
    async def test_initialize_aws_client_aws_mode(
        ...
        with patch('app.main.blueprints.deputy_dev.services.code_review.ide_review.base_websocket_manager.AWSAPIGatewayServiceClient') as mock_client_class, \
             patch('app.main.blueprints.deputy_dev.services.code_review.ide_review.base_websocket_manager.ConfigManager') as mock_config:
            
            mock_client = AsyncMock()
            mock_client_class.return_value = mock_client
            mock_config.configs = {
                'AWS_API_GATEWAY': {
                    'CODE_REVIEW_WEBSOCKET_WEBHOOK_ENDPOINT': 'wss://test-endpoint.com'
                }
            }
...
class TestBaseWebSocketManagerSendErrorMessage:
    ...
    async def test_send_error_message_success(
        ...
        await sample_local_websocket_manager.send_error_message(
            error_message, 
            sample_local_stream_buffer
        )
...
class TestBaseWebSocketManagerEdgeCases:
    ...
    async def test_multiple_simultaneous_progress_contexts(
        ...
        tasks = [
            asyncio.create_task(context_task(0.1)),
            asyncio.create_task(context_task(0.15))
        ]
...
class TestBaseWebSocketManagerIntegration:
    ...
    async def test_full_websocket_flow_aws(
        ...
        with patch('app.main.blueprints.deputy_dev.services.code_review.ide_review.base_websocket_manager.AWSAPIGatewayServiceClient') as mock_client_class, \
            ...
            mock_config.configs = {
                'AWS_API_GATEWAY': {
                    'CODE_REVIEW_WEBSOCKET_WEBHOOK_ENDPOINT': 'wss://test.com'
                }
            }
