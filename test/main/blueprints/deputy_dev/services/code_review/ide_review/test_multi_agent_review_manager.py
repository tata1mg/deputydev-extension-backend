...
class TestMultiAgentWebSocketManagerInitialization:
    ...
    def test_init_with_default_parameters(self) -> None:
        ...
        manager = MultiAgentWebSocketManager(connection_id="test-conn-123", review_id=456)
    ...
    def test_init_with_local_flag(self) -> None:
        ...
        manager = MultiAgentWebSocketManager(connection_id="local-test-conn", review_id=789, is_local=True)
...
class TestMultiAgentWebSocketManagerExecuteAgentTask:
    ...
    async def test_execute_agent_task_success(
        ...
        with patch(
            "app.main.blueprints.deputy_dev.services.code_review.ide_review.multi_agent_review_manager.IdeReviewManager"
        ) as mock_ide_manager:
...
class TestMultiAgentWebSocketManagerProcessRequest:
    ...
    async def test_process_request_single_agent(
        ...
        with patch.object(sample_multi_agent_manager, "execute_and_stream_agent") as mock_execute:
            ...
            await sample_multi_agent_manager.process_request(
                sample_agent_request_list_single, sample_local_stream_buffer
            )

            mock_execute.assert_called_once_with(sample_agent_request_list_single[0], sample_local_stream_buffer)
    ...
    async def test_process_request_multiple_agents(
        ...
        with patch.object(sample_multi_agent_manager, "process_multiple_agents_with_cache_pattern") as mock_process:
            ...
            await sample_multi_agent_manager.process_request(
                sample_agent_request_list_multiple, sample_local_stream_buffer
            )

            mock_process.assert_called_once_with(sample_agent_request_list_multiple, sample_local_stream_buffer)
...
class TestMultiAgentWebSocketManagerProcessMultipleAgentsWithCachePattern:
    ...
    async def test_process_multiple_agents_with_cache_establishing_and_utilizing(
        ...
        with (
            patch(
                "app.main.blueprints.deputy_dev.services.code_review.ide_review.multi_agent_review_manager.UserAgentRepository"
            ) as mock_repo,
            patch.object(sample_multi_agent_manager, "execute_and_stream_agent") as mock_execute,
        ):
            ...
            await sample_multi_agent_manager.process_multiple_agents_with_cache_pattern(
                sample_mixed_agent_requests, sample_local_stream_buffer
            )
    ...
    async def test_process_multiple_agents_only_cache_establishing(
        ...
        with (
            ...
            await sample_multi_agent_manager.process_multiple_agents_with_cache_pattern(
                sample_cache_establishing_agents, sample_local_stream_buffer
            )
    ...
    async def test_process_multiple_agents_only_cache_utilizing(
        ...
        with (
            ...
            await sample_multi_agent_manager.process_multiple_agents_with_cache_pattern(
                sample_cache_utilizing_agents, sample_local_stream_buffer
            )
...
class TestMultiAgentWebSocketManagerExecuteAndStreamAgent:
    ...
    async def test_execute_and_stream_agent_query_request(
        ...
        with (
            patch.object(sample_multi_agent_manager, "execute_agent_task") as mock_execute,
            patch.object(sample_multi_agent_manager, "push_to_connection_stream") as mock_push,
        ):
            ...
            await sample_multi_agent_manager.execute_and_stream_agent(
                sample_agent_request_query, sample_local_stream_buffer
            )
    ...
    async def test_execute_and_stream_agent_tool_use_response_request(
        ...
        with (
            ...
            await sample_multi_agent_manager.execute_and_stream_agent(
                sample_agent_request_tool_use_response, sample_local_stream_buffer
            )
...
class TestMultiAgentWebSocketManagerEdgeCases:
    ...
    async def test_process_request_with_empty_agent_list(
        ...
        with patch.object(sample_multi_agent_manager, "execute_and_stream_agent") as mock_execute:
            ...
            await sample_multi_agent_manager.process_request(empty_agent_list, sample_local_stream_buffer)
    ...
    async def test_concurrent_agent_execution(
        ...
        with (
            patch.object(
                sample_multi_agent_manager, "execute_and_stream_agent", side_effect=mock_execute_and_stream_agent
            ),
            patch(
                "app.main.blueprints.deputy_dev.services.code_review.ide_review.multi_agent_review_manager.UserAgentRepository"
            ) as mock_repo,
...
class TestMultiAgentWebSocketManagerIntegration:
    ...
    async def test_end_to_end_single_agent_flow(
        ...
        with (
            ...
            await sample_multi_agent_manager.process_request([sample_agent_request_query], sample_local_stream_buffer)
    ...
    async def test_end_to_end_multi_agent_flow(
        ...
        with (
            ...
            await sample_multi_agent_manager.process_request(sample_mixed_agent_requests, sample_local_stream_buffer)
...
class TestMultiAgentWebSocketManagerPerformance:
    ...
    async def test_large_number_of_agents(
        ...
        large_agent_list = [
            AgentRequestItem(
                agent_id=i, review_id=456, type=RequestType.QUERY, payload={"content": f"Test content {i}"}
            )
            for i in range(50)
        ]

        # Mock user agents for all requests
        mock_user_agents = [MagicMock(id=i, agent_name="CODE_COMMUNICATION") for i in range(50)]
        ...
        with (
            ...
            await sample_multi_agent_manager.process_multiple_agents_with_cache_pattern(
                large_agent_list, sample_local_stream_buffer
            )
