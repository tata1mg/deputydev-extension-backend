...
class TestBaseCodeReviewAgentInitialization:
    ...
    def test_init_single_pass_agent(
        self,
...
class TestBaseCodeReviewAgentTokenHandling:
    ...
    def test_get_tokens_data_with_empty_system_message(
        ...
        messages = UserAndSystemMessages(
            system_message=None,
    ...
    def test_has_exceeded_token_limit_false(
        ...
        mock_config.config = {
            "LLM_MODELS": {
                "GPT_4O": {"INPUT_TOKENS_LIMIT": 2000}
            }
        }
...
class TestBaseCodeReviewAgentRunAgent:
    ...
    async def test_run_agent_single_pass_success(
        ...
        with patch('app.main.blueprints.deputy_dev.services.code_review.ide_review.agents.base_code_review_agent.CONFIG') as mock_config:
            mock_config.config = {
                "LLM_MODELS": {
                    "GPT_4O": {"INPUT_TOKENS_LIMIT": 128000}
                }
            }
    ...
    async def test_run_agent_token_limit_exceeded(
        ...
        with patch('app.main.blueprints.deputy_dev.services.code_review.ide_review.agents.base_code_review_agent.CONFIG') as mock_config:
            ...
            with patch('app.main.blueprints.deputy_dev.services.code_review.ide_review.agents.base_code_review_agent.TikToken') as mock_tiktoken_class:
                mock_tiktoken_instance = MagicMock()
    ...
    async def test_run_agent_llm_handler_exception(
        ...
        with patch('app.main.blueprints.deputy_dev.services.code_review.ide_review.agents.base_code_review_agent.CONFIG') as mock_config:
            ...
            agent = TestAgent(
                context_service=mock_context_service,
                llm_handler=llm_handler_with_exception,
                model=LLModels.GPT_4O
            )
...
class TestBaseCodeReviewAgentIntegration:
    ...
    async def test_full_agent_workflow_single_pass(
        ...
        mock_prompt_handler.get_prompt.return_value = UserAndSystemMessages(
            system_message="You are a security code reviewer.",
            user_message="Review this code: def test(): pass"
        )
