...
def generate_conversation(payload: ChatModel.ChatRequestModel, context=""):
    ...
    """
    system_message = (f"{INSTRUCTIONS}\n" f"Context: {context}\n") if context else INSTRUCTIONS

    conversation = [{"role": "system", "content": system_message}]
    for chat in payload.chat_history:
        conversation.append({"role": chat.role, "content": chat.prompt})
    conversation.append({"role": "user", "content": payload.current_prompt})
    return conversation
...
async def cache_user_chat_history(
    payload: ChatModel.ChatRequestModel, serialized_response: ChatModel.ChatResponseModel, context=""
):
    """
    ...
    if payload.chat_type == "pdf":
        chat_history.append({"type": JivaChatTypes.ChatTypePdf.value, "role": "user", "prompt": context})
