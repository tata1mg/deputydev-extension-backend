...
class PlanCodeGenerationHandler(BaseFeatureHandler):
    def __init__(
        self,
    ...
    async def validate_and_set_final_payload(self) -> None:
        ...
        if not self.session_id:
            raise ValueError("Session ID is required for code generation from plan")
    ...
    async def handle_feature(self) -> FeatureHandlingResult:
        ...
        api_response = await self.one_dev_client.plan_code_generation(
            payload={},
            headers=headers,
        )
