...
class IdeReviewPreProcessor:
    def __init__(self):
        ...
        self.review_status = IdeReviewStatusTypes.IN_PROGRESS.value
        self.is_valid = True
    ...
    async def pre_process_pr(self, data: dict, user_team_id: int):
        ...
        diff_handler = IdeDiffHandler(review_diff)
    ...
    async def run_validation(self, review_diff: str, token_count: int):
        ...
        if not review_diff:
            self.is_valid = False
            self.review_status = IdeReviewStatusTypes.REJECTED_NO_DIFF.value
        elif token_count > MAX_PR_DIFF_TOKEN_LIMIT:
            self.is_valid = False
            self.review_status = IdeReviewStatusTypes.REJECTED_LARGE_SIZE.value
