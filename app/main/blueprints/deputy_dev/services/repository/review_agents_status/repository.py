from app.main.blueprints.deputy_dev.models.dao.postgres.review_agent_status import ReviewAgentStatus
...
class ReviewAgentStatusRepository:
    ...
    async def db_insert(cls, agent_status: ReviewAgentStatusDTO):
        ...
        try:
            ...
            del payload["id"]
            row = await DB.insert_row(ReviewAgentStatus, payload)
    ...
    async def db_get(
        ...
        try:
            agent_status_data = await DB.by_filters(
                model_name=ReviewAgentStatus, where_clause=filters, fetch_one=fetch_one, order_by=order_by
            )
