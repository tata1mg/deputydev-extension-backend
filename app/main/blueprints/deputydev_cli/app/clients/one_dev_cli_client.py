...
class OneDevCliClient(OneDevClient):
    async def generate_code(self, payload: Dict[str, Any], headers: Dict[str, str]) -> Dict[str, Any]:
        ...
        result = await self.post(self._host + path, json=payload, headers={**headers, "X-Client-Version": "1.5.0", "X-Client": "CLI"})
    ...
    async def generate_docs(self, payload: Dict[str, Any], headers: Dict[str, str]) -> Dict[str, Any]:
        ...
        result = await self.post(url=self._host + path, json=payload, headers={**headers, "X-Client-Version": "1.5.0", "X-Client": "CLI"})
        return (await result.json()).get("data")
    ...
    async def get_registered_repo_details(self, payload: Dict[str, Any], headers: Dict[str, str]) -> Dict[str, Any]:
        ...
        result = await self.get(url=self._host + path, headers={**headers, "X-Client-Version": "1.5.0", "X-Client": "CLI"}, params=payload)
        return (await result.json()).get("data")
    ...
    async def verify_auth_token(self, headers: Dict[str, str]) -> Dict[str, Any]:
        ...
        result = await self.post(url=self._host + path, headers={**headers, "X-Client-Version": "1.5.0", "X-Client": "CLI"})
        return (await result.json()).get("data")
