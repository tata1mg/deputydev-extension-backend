...
class OpenAIEmbeddingManager(BaseEmbeddingManager):
    ...
    @classmethod
    async def embed_text_array(cls, texts: Tuple[str], store_embeddings: bool = True) -> Tuple[List[np.ndarray], int]:
        """
        ...
        for idx, batch in enumerate(batches):
            ...
            if len(parallel_tasks) >= max_parallel_tasks:
                data = await asyncio.gather(*parallel_tasks)
                parallel_tasks = [OpenAILLMService().get_embeddings(batch, store_embeddings)]
                for d in data:
                    embeddings.extend(d[0])
                    input_tokens += d[1]
            else:
                parallel_tasks.append(OpenAILLMService().get_embeddings(batch, store_embeddings))
