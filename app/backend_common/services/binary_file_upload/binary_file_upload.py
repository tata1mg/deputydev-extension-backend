...
class BinaryFileUpload:
    s3_client = AWSS3ServiceClient(
        bucket_name=ConfigManager.configs["AWS_S3_BINARY_BUCKET"]["AWS_BUCKET_NAME"],
        region_name=ConfigManager.configs["AWS_S3_BINARY_BUCKET"]["AWS_REGION"],
    )

    PRESIGNED_URL_EXPIRY = ConfigManager.configs["BINARY_UPLOAD"]["PRESIGNED_URL_EXPIRY"]

    CACHE_CONTROL = ConfigManager.configs["BINARY_UPLOAD"]["CACHE_CONTROL"]
    ...
    async def get_presigned_urls_for_upload(cls, s3_key: str) -> str:
        ...
        """
        presigned_post_url = await cls.s3_client.create_presigned_put_url(
            expiry=cls.PRESIGNED_URL_EXPIRY, s3_key=s3_key, cache_control=cls.CACHE_CONTROL
        )
