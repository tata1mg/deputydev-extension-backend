...
from app.backend_common.utils.sanic_wrapper.clients import TorpedoClient
...
class ServiceResponseHealthCheck(HealthCheckStrategy):
    async def check(self) -> str:
        try:
            result = await TorpedoClient.get(path="/ping", purge_response_keys=True)
            if result.status != 200:
                error_logger.error(SERVICE_RESPONSE_HEALTH_CHECK_FAILED)
                raise BaseTorpedoException(SERVICE_RESPONSE_HEALTH_CHECK_FAILED)
            return "healthy"
